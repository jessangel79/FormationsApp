//
//  WebsiteFormationViewController.swift
//  FormationsApp
//
...
final class WebsiteFormationViewController: UIViewController, WKUIDelegate {
    ...
    @IBAction private func safariBarButtonItemTapped(_ sender: UIBarButtonItem) {
        guard let urlCellule = cellule?.webSite else { return }
        openSafari(urlCellule)
    }
    
    @IBAction private func shareBarButtonItemTapped(_ sender: UIBarButtonItem) {
        guard let website = cellule?.webSite else { return }
    ...
    override func viewDidLoad() {
        ...
        loadWebsite(cellule?.webSite ?? "", webView: webView)
    }
